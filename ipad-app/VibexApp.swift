...
/// 시험용 진입점.
///
/// 실제 화면 흐름(연결 → 목록 → 캔버스 → 확인 → 결과)이 붙기 전까지, 캔버스와
/// 전송 계층을 실기기에서 만져 보기 위한 최소 껍데기다. 본 화면들이 생기면
/// 이 파일은 지워도 된다.
@main
struct VibexApp: App {
    var body: some Scene {
...
struct HarnessView: View {
    ...
    var body: some View {
        ...
        // 자동 확인용. `simctl launch <device> com.vibexipad -openCanvas` 로 바로 캔버스를 연다.
        .onAppear {
            if ProcessInfo.processInfo.arguments.contains("-openCanvas") { showCanvas = true }
        }
...
/// 주석 대상으로 쓸 가짜 로그인 화면. 실제 스크린샷이 붙기 전까지의 대역이다.

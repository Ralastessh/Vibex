...
// No custom chrome: the panel's title bar, its actions (new conversation,
// history) and the tab itself are rendered natively by VS Code via the
// view/title menu contributions in package.json.
...
// Secondary row: session target pill (extensions icon + VIBEX), then the two
// option pickers inside one chat-sessionPicker-container, as in the original.
const sessionPill = el("li", "action-item chat-input-picker-item chat-session-target-picker-item");
const sessionDropdown = el("div", "monaco-dropdown");
const sessionDropdownLabel = el("div", "dropdown-label");
const sessionAnchor = el("a", "action-label compact");
